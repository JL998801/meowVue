...
let isUpdating = false; // 這個變數用來避免 `watch` 無限觸發，不然更新完後，又改變父組件image，導致圖片預覽不正確
...
const handleImageUploaded = ({ frontTmpUrl, backTmpUrl }) => {
  ...
  isUpdating = true; //避免 `watch` 觸發
  image.value = frontTmpUrl; // 每次上傳都會覆蓋之前的圖片，導致最終只有一張
  emit("image-uploaded", backTmpUrl); //傳遞給父組件新增案件contoller使用的圖片URL
  setTimeout(() => (isUpdating = false), 100); // 避免 `watch` 太快觸發
  closeUploadModal();
};

watchEffect(() => {
  if (!isUpdating && props.imageUrl && props.imageUrl !== image.value) {
    console.log("檢測到 imageUrl 變化:", props.imageUrl);
